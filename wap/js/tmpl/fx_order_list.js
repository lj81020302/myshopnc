...
$(function(){
	...
	if(!key){
		window.location.href = WapSiteUrl+'/tmpl/fx_member/login.html';
	}
	...
	function initPage(){
		...
		$.ajax({
			type:'post',
			url:ApiUrl+"/index.php?act=fx_member_order&op=order_list&page="+page+"&curpage="+curpage,
    ...
    $('#order-list').on('click','.check-payment',function() {
        ...
        toPay(pay_sn,'fx_member_buy','pay');
        return false;
    });
    ...
    function cancelOrderId(order_id) {
        $.ajax({
            type:"post",
            url:ApiUrl+"/index.php?act=fx_member_order&op=order_cancel",
