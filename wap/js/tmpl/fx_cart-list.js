$(function (){
    ...
    if(!key){
        var goods = decodeURIComponent(getCookie('fx_goods_cart'));
        if (goods != null) {
        ...
        $(".goods-del").click(function(){
            ...
            $.sDialog({
                ...
                okFn: function() {
                    ...
                    addCookie('fx_goods_cart',goodsarr.join('|'));
                    // 更新cookie中商品数量
                    addCookie('fx_cart_count',goodsarr.length-1);
                    location.reload();
                }
            });
        });
        ...
        function initCartList(){
             $.ajax({
                url:ApiUrl+"/index.php?act=fx_member_cart&op=cart_list",
                type:"post",
                dataType:"json",
                data:{key:key},
        ...
        function delCartList(cart_id){
            $.ajax({
                url:ApiUrl+"/index.php?act=fx_member_cart&op=cart_del",
                type:"post",
        ...
        function editQuantity(self,type){
            ...
            $.ajax({
                url:ApiUrl+"/index.php?act=fx_member_cart&op=cart_edit_quantity",
                type:"post",
        ...
        $('#cart-list-wp').on('click', ".check-out > a", function(){
            ...
            window.location.href = WapSiteUrl + "/tmpl/order/fx_buy_step1.html?ifcart=1&cart_id="+cart_id;
        });
...
function calculateTotalPrice() {
    ...
    $('.cart-litemw-cnt').each(function(){
        if ($(this).find('input[name="cart_id"]')) {
...
function getGoods(goods_id, goods_num){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+'/index.php?act=fx_goods&op=goods_detail&goods_id='+goods_id,
        dataType:'json',
        async:false,
        success:function(result){
            if (result.datas.error) {
            ...
            data.store_id = result.datas.agent_id;
            data.store_name = result.datas.agent_name;
            data.goods_id = goods_id;
            data.goods_name = result.datas.goods_name;
            data.goods_price = result.datas.goods_price;
            data.goods_num = goods_num;
            data.goods_image_url = pic[0];
            data.goods_sum = (parseInt(goods_num)*parseFloat(result.datas.goods_price)).toFixed(2);
        }
    });
    return data;
}
...
function get_footer() {
        footer = true;
        $.ajax({
            url: WapSiteUrl+'/js/tmpl/fx_footer.js',
            dataType: "script"
          });
}
...
//S7 添加登录后，返回购物车
function car_login(){
	addCookie('redirect_uri','/tmpl/fx_member/cart_list.html');
	window.location.href='login.html';
}
function car_register(){
	addCookie('redirect_uri','/tmpl/fx_member/fx_cart_list.html');
