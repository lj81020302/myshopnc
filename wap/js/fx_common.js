...
function checkLogin(state){
	if(state == 0){
		location.href = WapSiteUrl+'/tmpl/fx_member/login.html';
		return false;
	}else {
		return true;
	}
...
function buildUrl(type, data) {
    switch (type) {
        ...
        case 'goods':
            return WapSiteUrl + '/tmpl/fx_member/product_detail.html?goods_id=' + data;
        case 'url':
            return data;
    }
    return WapSiteUrl;
...
function updateCookieCart(key) {
    var cartlist = decodeURIComponent(getCookie('fx_goods_cart'));
    if (cartlist) {
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?act=fx_member_cart&op=cart_batchadd',
            data:{key:key, cartlist:cartlist},
            dataType:'json',
            async:false
        });
        delCookie('fx_goods_cart');
    }
}
/**
 * 查询购物车中商品数量
...
function getCartCount(key, expireHours) {
    var cart_count = 0;
    if (getCookie('key') !== null && getCookie('fx_cart_count') === null) {
        var key = getCookie('key');
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?act=fx_member_cart&op=cart_count',
            data:{key:key},
            dataType:'json',
            async:false,
            success:function (result) {
                if (typeof(result.datas.cart_count) != 'undefined') {
                    addCookie('fx_cart_count',result.datas.cart_count,expireHours);
                    cart_count = result.datas.cart_count;
                }
            }
        });
    } else {
        cart_count = getCookie('fx_cart_count');
    }
